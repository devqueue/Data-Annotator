...
def upload_file(request):
    ...
    if form.is_valid():
        ...
        if df1.columns[0] != "chromosome":
            df1.columns = [i for i in range(0, len(df1.columns))]
            chrom_index = df1.loc[df1[0].str.contains("chromosome", case=False)].index[0]
            df1 =  df1[chrom_index:]
            df1.columns = df1.loc[chrom_index]
            df1 = df1.drop(chrom_index)
            df1 = df1.reset_index()
        ...
        try:
            ...
            df1 = df1.drop(df1.columns.difference(req), axis=1)
